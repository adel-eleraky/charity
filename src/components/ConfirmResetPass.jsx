...
import {
  confirmResetPass,
  forgotPassword,
} from "../rtk/features/user/userAuthSlice";
...
function ConfirmResetPass() {
  const { error, confirmResetPassStatus } = useSelector(
  ...
  const validationSchema = yup.object().shape({
    ...
    password: yup.string().required("ادخل كلمة السر"),
  });
  ...
  useEffect(() => {
    ...
    if (confirmResetPassStatus === "loading")
      toast.loading("جارى تغيير كلمة المرور");
    ...
    if (confirmResetPassStatus === "failed")
          ...
          >
            {({ values, errors, touched }) => {
              return (
                <Form
                  method="post"
                  className="needs-validation reset-password-form p-2 rounded w-75 m-auto"
                  noValidate
                >
                  ...
                  <button
                    type="submit"
                    className="btn d-block m-auto submit-btn"
                  >
                    حفظ كلمة المرور
                  </button>
                </Form>
